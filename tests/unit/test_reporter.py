...
def _alert(ip: str, event_type: str, severity: str = "high", payload: dict | None = None):
    ...
    a.timestamp = datetime(2024, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
    return a
...
async def test_html_report_escapes_malicious_event_type():
    ...
    html = await generate(
        title="Test", alerts=alerts, stats={}, target_ip=None, format="html"
    )
...
async def test_markdown_report_escapes_pipes_in_cells():
    ...
    md = await generate(
        title="Test", alerts=alerts, stats={}, target_ip=None, format="markdown"
    )
...
async def test_html_report_with_no_alerts_renders():
    html = await generate(
        title="Empty Report", alerts=[], stats={}, target_ip=None, format="html"
    )
